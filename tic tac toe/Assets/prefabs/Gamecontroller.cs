...
public class Gamecontroller : MonoBehaviour {
	...
	ArrayList filterboard(int [] board)
	{
	...
	public int min_max(int[] board, string symbol)
	{
		int bestval = -1000;
		int bestmove = -1;
	...
	public int mini_max(int[] board, string symbol)
	{
		int nsymbol = (symbol == "X" ? 1 : 2);
		...
		if (symbol==compside)
		{
			...
			{
				board[(int)eboard[i]] = nsymbol;
				tbest = mini_max(board, changeside(symbol));
				if (tbest > best)
					best = tbest;
				board[(int)eboard[i]] = 0;
			}
	...
	void gameover (int win)
	{
		...
		else 
		{
			...
			if (win==1) 
			{
				gameovertext.text = name1;
				gameoverpanel.SetActive (true);
			} 
			else if (win==2)
			{
				gameovertext.text = name2;
				gameoverpanel.SetActive (true);	
			}
		}
	...
	int checkturn(int[] board,string symbol)
	{
		ArrayList emptyboard = filterboard (board);
		int j,i;
		...
		{
			player = checkboard (board,symbol);
			for (j = 0; j < 5; j++)
				if (player [j] == 0)
					break;
	...
	public int[] checkboard(int[] board,string symbol)
	{
